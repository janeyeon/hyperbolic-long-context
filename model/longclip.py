...
from .model_longclip import build_model_hyco, build_model
...
def _transform_hyco(image_size):
    return Compose([
...
def load_from_clip(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, is_hycoclip = None, train_config = None, load_from_clip:bool = True):
    ...
    def _download(url: str, root: str, is_hycoclip:bool):

        if is_hycoclip: 
            filename = os.path.basename(url)
            expected_sha256 = url.split("/")[-2]
            #? 그게 아니라면 
            download_target = root
        else: 
    ...
    if is_hycoclip:
        model = build_model_hyco(state_dict or model.state_dict(), load_from_clip = load_from_clip).to(device)
    else:
    ...
    if is_hycoclip:
        return model, _transform_hyco(224)
    else:
        return model, _transform(model.input_resolution.item())
