...
class TransformerTextEncoder(nn.Module):
    ...
    def __init__(
        ...
        if load_from_clip:
            self.posit_embed = nn.Parameter(torch.empty(77, self.width))
        else:
            self.posit_embed = nn.Parameter(torch.empty(context_length, self.width))
        self.posit_embed_res = nn.Parameter(torch.empty(context_length, self.width))
        ...
        # )
        if load_from_clip:
            #! 이떄는 시작이 77 token으로 시작함 
            attn_mask = torch.triu(
                torch.full((77, 77), float("-inf")), diagonal=1
            )
        else:
            attn_mask = torch.triu(
                torch.full((context_length, context_length), float("-inf")), diagonal=1
    ...
    def forward(self, text_tokens: torch.Tensor, attn_mask) -> torch.Tensor:
        """
