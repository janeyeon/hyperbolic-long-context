...
class CLIP_hyco(nn.Module):
    def __init__(self,
                embed_dim: int,
                 ...
                 textual_arch = "L12_W512",
                 textual_vocab_size = 49408,
                 textual_context_length = 248,
                 load_from_clip:bool = True
                ):

        super().__init__()

        self.context_length = textual_context_length
        # self.context_length = 248

        self.visual = build_timm_vit(
            arch = vision_arch,
            global_pool = vision_global_pool,
            use_sincos2d_pos = vision_use_sincos2d_pos
        )

        self.textual = TransformerTextEncoder(
            arch = textual_arch,
            vocab_size = textual_vocab_size,
            context_length = textual_context_length, 
            load_from_clip=load_from_clip
        )
        self.embed_dim = embed_dim
        self.initialize_parameters()
        transformer_width = transformer_width
        self.vocab_size = self.textual.vocab_size
        # self.token_embed = nn.Embedding(self.vocab_size, transformer_width)

        if load_from_clip == False:
            self.textual.posit_embed = nn.Parameter(torch.empty(248, transformer_width))
            self.textual.posit_embed_res = nn.Parameter(torch.empty(248, transformer_width))

        else:
            self.textual.posit_embed = nn.Parameter(torch.empty(77, transformer_width))

        self.textual.ln_final = LayerNorm(transformer_width)
        self.textual_proj.weight = nn.Parameter(torch.empty(transformer_width, embed_dim))
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
    ...
    def initialize_parameters(self):
        ...
        self.visual_proj = nn.Linear(self.visual.width, self.embed_dim, bias=False)
        self.textual_proj = nn.Linear(self.textual.width, self.embed_dim, bias=False)
    ...
    def encode_text_full(self, tokens: list[torch.Tensor]):
        ...
        for block in self.textual.resblocks:
            ...
            if self.grad_checkpointing and self.training:
                ...
                x = block(x, attn_mask)
        x = self.textual.ln_final(x).type(self.dtype)
        return x
...
def convert_weights_hyco(model: nn.Module):
    """Convert model parameters to fp16 (CLIP / timm / Meta-CLIP safe)"""
    ...
    def _convert(m: nn.Module):
        ...
        # 4. MultiheadAttention (Meta CLIP style)
        if isinstance(m, nn.MultiheadAttention):
            for attr in [
                ...
                if obj is not None:
                    obj.data = obj.data.half()

        # 5. Projection layers (custom names)
        for name in ["visual_proj", "textual_proj"]:
            if hasattr(m, name):
                proj = getattr(m, name)
                if isinstance(proj, nn.Linear):
                    proj.weight.data = proj.weight.data.half()
                    if proj.bias is not None:
                        proj.bias.data = proj.bias.data.half()

        # 6. Positional embeddings / masks (Parameter)
        for name in ["pos_embed", "pos_embed_res", "mask1", "mask2"]:
            if hasattr(m, name):
                p = getattr(m, name)
...
def build_model_hyco(state_dict: dict, load_from_clip: bool):
    ...
    vision_layers = len({
        k.split(".")[1]
        for k in state_dict
        if k.startswith("visual.blocks.") and k.endswith(".attn.qkv.weight")
    })

    vision_patch_size = state_dict["visual.patch_embed.proj.weight"].shape[-1]

    num_tokens = state_dict["visual.pos_embed"].shape[1]
    ...
    vocab_size = state_dict["textual.token_embed.weight"].shape[0]

    transformer_width = state_dict["textual.token_embed.weight"].shape[1]
    embed_dim = transformer_width

    context_length = state_dict["textual.posit_embed"].shape[0]

    transformer_layers = len({
        k.split(".")[2]
        for k in state_dict
        if k.startswith("textual.resblocks.") and ".attn.in_proj_weight" in k
    })
    ...
    model = CLIP_hyco(
        embed_dim, 
        image_resolution, vision_layers, vision_width, vision_patch_size,
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers, vision_arch ="vit_base_patch16_224", vision_global_pool= "token", vision_use_sincos2d_pos = True, textual_arch="L12_W512", textual_vocab_size=49408, textual_context_length=248, load_from_clip = load_from_clip
    )
